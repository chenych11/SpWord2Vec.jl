...
export SparseSkipGramBb, train, init, close_device
...
type SparseSkipGramBb{Tensor} # Tensor could be HostArray{Float32, 2}
    α::Tensor
    β::Tensor
    B::Tensor
    C::Tensor
    ...
    function SparseSkipGramBb(d::Integer, # vector dimension
...
function save(model::SparseSkipGramBb{Array}, fname::String; save_maps=false)
...
num_vocab(model::SparseSkipGramBb) = size(model.α, 2)
num_base(model::SparseSkipGramBb) = size(model.α, 1)
num_vecdim(model::SparseSkipGramBb) = size(model.B, 1)
num_minibatch(model::SparseSkipGramBb) = size(model.a⃗, 2)
num_negative(model::SparseSkipGramBb) = size(model.b⃗, 2) - 1

function to_device{M <: Union{SparseSkipGramBb{Array},
                    SparseSkipGramBb{HostArray}}}(model::M)
    d = num_vecdim(model)
    ...
    d_model = SparseSkipGramBb{CudaArray}(d, num_base_, num_vocab_, mini_batch,
...
@generated function get_training_model(model::SparseSkipGramBb)
    if BACKEND == "GPU"
...
function gpu_sparse_update!(arr, grad, idxes, lr, clip)
    # make sure the transferring completes.
    CUDArt.synchronize(CUDArt.null_stream)
    ccall((:sparse_update_clip, vec_lib), Void, (Ptr{Float32}, Ptr{Float32},
        Cint, Cint, Ptr{Cuint}, Float32, Float32, Ptr{Void}, Cint),
        arr.ptr, grad.ptr, size(grad, 1), size(grad, 2),
        idxes.ptr, lr, clip, null_stream.inner.handle, 256)
end
...
@generated function proximal_update!(model, ∇αβ, idxes)
    if BACKEND == "GPU"
        quote
            ...
            copy!(d_idxes, idxes)
            gpu_sparse_update!(model.α, d_∇αβ, d_idxes, lr_, model.λ*model.lr0)
        end
    else
        quote
