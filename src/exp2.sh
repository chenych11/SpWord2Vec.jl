...
corpus="$DATA_DIR/pairs-wiki-1based-20k.idx"
...
idx=34
for model in A B; do
    savename=$model.lr${lr}_${minlr}.λ${lmd}.neg${neg}.DLine${datapp}.nrm_dict.pre${prer}.U${every}
    echo "julia main.jl --model=$model --lr=$lr --min-lr=${minlr} --lambda=${lmd} --negative=${neg} --pretrain=${prer} --every=${every} --part=${datap} --normalize-dict --save-name=$idx-$savename  --corpus=$corpus 2>&1 | tee \"$DATA_DIR/log/$idx-${savename}.log\""
    let idx+=1
done
